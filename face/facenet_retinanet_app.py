...
import os
import json
from facenet_retinanet_utils import recognition_name, identification_face, face_encoding
...
def face_reg():
    try:
        ...
        with open('reg_student_json3.json') as f1:
            all_json_str = json.load(f1)
        with open('reg_student_json3.json', 'w') as f2:
        ...
        #new_encodings = face_encoding(f)
        if os.path.exists('new_data3.npz'):
            npz = np.load('new_data3.npz')
        ...
        np.savez('new_data3.npz', encode=known_face_encodings, sids=sids, names=known_face_names)
