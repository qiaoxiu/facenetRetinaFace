...
import os
import json
from dlib_utils import recognition_name, identification_face, face_encoding
...
def face_reg():
    try:
        ...
        with open('reg_student_json.json') as f1:
            all_json_str = json.load(f1)
        with open('reg_student_json.json', 'w') as f2:
        ...
        #new_encodings = face_encoding(f)
        if os.path.exists('new_data.npz'):
            npz = np.load('new_data.npz')
            known_face_encodings = npz['encode']
            sids = npz['sids']
            known_face_names = npz['names']
            flag, num = 0, known_face_encodings.shape[0]
            #known_face_encodings = np.vstack((encodes, new_encodings[:,np.newaxis].T))
            for i in range(num):
                if str(sids[i])==sid:
        ...
        np.savez('new_data.npz', encode=known_face_encodings, sids=sids, names=known_face_names)
